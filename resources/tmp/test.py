...
def test():
    ...
    for loop_idx in ["1000", "2000", "3000", "4000", "5000"]:
        ...
        with torch.no_grad():
            ...
            logging.warning(f"step:{loop_idx} T-model 准确率：{point/len(true_batch)}  point:{point} in {len(true_batch)}")

        s_model = ERMStudent(dataset.input_shape, dataset.num_classes, num_domains=3, hparams=config.hparams).to(device)
        ...
        with torch.no_grad():
            ...
            for i_batch, o_batch in zip(i_batches, o_batches):
                ...
                point += sum(predict_rst.argmax(1)==o_batch).item()
            logging.warning(f"step:{loop_idx} S-model 准确率：{point/len(true_batch)}  point:{point} in {len(true_batch)}")
