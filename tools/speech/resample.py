...
def resample(file_path, rst_file_path, target_sr):
    ...
    audio_hat = librosa.resample(
        audio, orig_sr=sr, target_sr=target_sr, fix=True, scale=False
    )
...
if __name__ == "__main__":
    wav_filenames = [
        file_name
        for file_name in os.listdir(RAW_DIR_PATH)
    ...
    with Pool(processes=PROCESS_NUM) as pool:
        result = list(
            tqdm(
                pool.imap(
                    partial(_resample, target_sr=TARGET_SR),
                    list(zip(raw_paths, dst_paths)),
                ),
                total=len(raw_paths),
            )
        )
