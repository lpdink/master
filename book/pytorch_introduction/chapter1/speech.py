...
def read_waves(file_path):
    ...
    length = int.from_bytes(head[4:8], "little") # 文件长度 unit32 int.from_bytes(length, 'little') 2**32B为4GB，故wav文件不能超过4G。
    ...
    code_rate = int.from_bytes(head[28:32], "little") # 码率 采样率x位深度x通道数/8 比如双通道的44.1K 16位采样的码率为176400
