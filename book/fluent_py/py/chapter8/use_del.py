...
if __name__ == "__main__":
    ...
    print(
        f"funcs1 is funcs2? {funcs1 is funcs2} funcs1==funcs2?{funcs1==funcs2}"
    )  # False，两个list是独立的
    ...
    print(
        f"test's obj is dead?(after del test): {test_weak_ref() is None}"
    )  # False，test's obj没有被回收
    del method
    print(
        f"test's obj is dead?(after del method): {test_weak_ref() is None}"
    )  # True,删除方法后，obj被回收了。可见对于集合对象，如果集合内的任何一个对象没被回收，集合就不会被回收。
