...
FILE_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "../resources/space.txt"
)
...
def read_space():
    ...
    return (
        SPACE,
        (np.where(SPACE == 2)[0][0], np.where(SPACE == 2)[1][0]),
        (np.where(SPACE == 3)[0][0], np.where(SPACE == 3)[1][0]),
    )
...
class Ant:
    ...
    def move(self):
        ...
        for next_pos in possible_next_pos:
            ...
            dis_score = (
                1 / abs_distance((new_x, new_y), (END[0], END[1]))
                if next_pos != END
                else 100
            )
...
def main():
    ...
    for loop_idx in range(config.loop):
        ...
        for _ in range(config.ants_num):
            ...
            while ant_loop:
                ...
                if path is not None:
                    ...
                    if (
                        loop_minium_path is None
                        or path_length < loop_minium_path_length
                    ):
        ...
        if loop_minium_path is not None:
            ...
            if global_minium_path is None or loop_minium_path_length < get_path_length(
                global_minium_path
            ):
